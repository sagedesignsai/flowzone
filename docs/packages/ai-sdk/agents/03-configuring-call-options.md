...
When you need agent behavior to change based on runtime context, call options eliminate the need to create multiple agents or handle configuration logic outside the agent.

## How It Works

Define call options in three steps:

1. Define `callOptionsSchema` - Zod schema for your options
2. Implement `prepareCall` - Function to modify agent settings
3. Pass `options` to `generate()` or `stream()`
...
The `options` parameter is now required and type-checked by TypeScript.
...
**Source:** [Configuring Call Options - AI SDK](https://ai-sdk.dev/docs/agents/configuring-call-options)
