/**
 * Marketing Agent Chat Handler
 *
 * Routes chat requests to the Marketing agent with
...
import { createMarketingAgent } from "@/lib/agents/marketing-agent"
...
export async function handleMarketingChat(options: {
  chatId: string
  userId: string
  ...
  if (!sandboxResult.ok) {
    logger.warn("No sandbox available for Marketing Agent", {
      chatId,
    ...
    return Response.json(
      {
        error:
          "A sandbox is required for marketing file operations. " +
  ...
  const agent = createMarketingAgent(model, extraTools)
